...
### #017 Vercel 环境变量保存后需重新部署才能生效

**场景**：在 Vercel Environment Variables 页面添加/修改环境变量后，直接访问 API 仍使用旧值。

**现象**：保存 DATABASE_URL 后，API 仍报数据库连接失败；新添加的 CRON_SECRET 不生效。

**原因**：Vercel 的环境变量只在**部署时**注入，已完成的部署实例不会自动更新环境变量。

**解决**：
1. 保存环境变量后，点击页面提示的 **Redeploy** 按钮
2. 或手动进入 Deployments → 选择最新部署 → 点击 **...** → **Redeploy**
3. 等待新部署完成（Status 变为 Ready）

**建议**：每次修改环境变量后，必须重新部署才能生效。

------

### #018 Vercel Python Serverless Function 返回 404

**场景**：部署 FastAPI 后端到 Vercel，访问 `/api/v1/players` 返回 404。

**现象**：Vercel Status 显示 Ready，但所有 API 路径都返回 404 NOT_FOUND。

**原因**：Vercel 旧版配置（`builds` + `routes`）不再被新版 Vercel 识别，Python Serverless Function 未正确部署。

**解决**：
1. 更新 vercel.json 为新版配置：
   ```json
   {
     "functions": {
       "api/index.py": {
...
3. 确保 Build Command、Output Directory、Install Command 都留空
4. 重新部署

**建议**：Vercel Python Runtime 需要使用新版 `functions` + `rewrites` 配置，旧版 `builds` + `routes` 已废弃。
...
| Cloudflare | Workers 不支持 Python 原生扩展；Python 后端需外部部署；Pages 的 wrangler.toml 不要包含 `[site]` 或 `[functions]`；创建时注意区分 Pages 和 Workers |
| Vercel | 环境变量保存后需重新部署才能生效；Python Runtime 需使用新版 `functions` + `rewrites` 配置；Framework Preset 应设为 Other |
